...
private int m_intLanguageID;
		private Dictionary<int, SqlString> m_strRoomName;
	...
	/// <summary>
		...
		public Rooms() {
			...
			m_strRoomName = new Dictionary<int, SqlString>();				
		...
		/// <summary>
		/// Gets all Rooms from the database, using the default connection string, into a SQLDataReader
		/// </summary>
		/// <returns>The SQLDataReader With the Rooms</returns>
		public static SqlDataReader GetAllRoomsReader(int LanguageID) {
			SqlConnection conn = DBHelper.getConnection();
			SqlCommand cmd = DBHelper.getSprocCmd("proc_Rooms__sel_all", conn);
		...
		public static DataSet GetAllRoomsDataSet(int LanguageID) {
            SqlConnection conn = DBHelper.getConnection();
            SqlCommand cmd = DBHelper.getSprocCmd("proc_Rooms__sel_all", conn);
		...
		/// <summary>
		/// Gets all Rooms from the database, using the default connection string, into a hashtable SQLReader
		/// </summary>
		/// <returns></returns>
		public static Hashtable GetAllRooms(int LanguageID) {
			SqlDataReader dr = GetAllRoomsReader(LanguageID);
			return ConvertReaderToHashTable(dr, LanguageID);
		}
		
		/// <summary>
		/// Creates Rooms for records In the prefilled DataReader, And puts them into a HashTable
		/// </summary>
		/// <param name="dr">The DataReader prefilled With the Rooms records</param>
		/// <returns>The Hashtable containing Rooms objects And their ID As key.</returns>
		private static Hashtable ConvertReaderToHashTable(SqlDataReader dr, int LanguageID) {
			Hashtable result = new Hashtable();
			while (dr.Read()) {
	            Rooms myRooms = new Rooms(LanguageID);
            
            myRooms.m_intRoomID = dr.GetSqlInt32(0);
            myRooms.m_intHotelID = dr.GetSqlInt32(1);
            myRooms.m_intCapacity = dr.GetSqlInt32(2);
            myRooms.m_dtCreateDate = dr.GetSqlDateTime(3);
            myRooms.m_intCreatedBy = dr.GetSqlInt32(4);
            myRooms.m_dtModifyDate = dr.GetSqlDateTime(5);
            myRooms.m_intModifiedBy = dr.GetSqlInt32(6);
            myRooms.m_bolisActive = dr.GetSqlBoolean(7);
            
            result.Add(myRooms.RoomID, myRooms);
		}
		...
		/// <summary>
		/// The RoomName column from ML table
		/// </summary>
		public string RoomName {
			get { 
				return this.GetRoomName(this.m_intLanguageID); 
			}
			set { 
				this.SetRoomName(value, this.m_intLanguageID);
		...
		/// The RoomName column from ML table
		/// </summary>
		public string GetRoomName(int LanguageID) {
			return this.m_strRoomName[LanguageID].Value;
		}
		
		/// <summary>
		/// The RoomName column from ML table
		/// </summary>
		public void SetRoomName(string RoomName, int LanguageID) {
			this.m_strRoomName[LanguageID] = RoomName;
		}
	...
	/// <returns>true if success</returns>
		public bool Load(Int32 intRoomID) {	
			//	construct new connection and command objects
			SqlConnection conn = DBHelper.getConnection();
			SqlCommand cmd = DBHelper.getSprocCmd("proc_Rooms__sel", conn);
		...
		//	open connection
				...
				if(reader.Read()) {
					...
					//	set member values
		            m_intRoomID = reader.GetSqlInt32(0);
            m_intHotelID = reader.GetSqlInt32(1);
            m_intCapacity = reader.GetSqlInt32(2);
            ...
            m_bolisActive = reader.GetSqlBoolean(7);
		
			
				cmd = DBHelper.getSprocCmd("proc_Rooms__sel_ml_data", conn);
            ...
            cmd.Parameters.Add(param);
				...
				//check if  anything was found
				while(reader.Read()) { 			
			m_strRoomName.Add((int)reader.GetSqlInt32(1), (SqlString)reader.GetSqlString(2));			
		...
		private int ins() {
			...
			if(this.m_Transaction != null) {
							cmd = DBHelper.getSprocCmd("proc_Rooms__ins", conn, this.m_Transaction);
			} else {
							cmd = DBHelper.getSprocCmd("proc_Rooms__ins", conn);
			}
        ...
        cmd.Parameters.Add(param);
			...
			conn.Close();
						...
						Rooms_ML.Insert(retValue, langID, this.m_strRoomName[langID].ToString(), this.m_strDescription[langID].ToString());
					}
		...
		private int upd() {
			...
			SqlCommand cmd;
			if(this.m_Transaction != null) {
				cmd = DBHelper.getSprocCmd("proc_Rooms__upd", conn, this.m_Transaction);
			} else {
				cmd = DBHelper.getSprocCmd("proc_Rooms__upd", conn);
			}
        ...
        cmd.Parameters.Add(param);
			...
			conn.Close();
								...
								Rooms_ML objRooms_ML = new Rooms_ML();
					foreach(int langID in this.m_strDescription.Keys) {
						objRooms_ML.RoomID = (int)this.m_intRoomID;
						objRooms_ML.LanguageID = langID;
						objRooms_ML.RoomName = this.m_strRoomName[langID].ToString();
objRooms_ML.Description = this.m_strDescription[langID].ToString();
 objRooms_ML.Update();
					}
		...
		private int del() {
			...
			SqlCommand cmd;
			if(this.m_Transaction != null) {
				cmd = DBHelper.getSprocCmd("proc_Rooms__del", conn, this.m_Transaction);
			} else {
				cmd = DBHelper.getSprocCmd("proc_Rooms__del", conn);
			}
		...
		/// <returns>true if success</returns>
		public static bool operator ==(Rooms t1, Rooms t2) {
			//	compare values
