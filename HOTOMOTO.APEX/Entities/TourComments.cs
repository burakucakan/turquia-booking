...
namespace HOTOMOTO.APEX {
	...
	public class TourComments {
		...
		//	members from table
		private SqlInt32 m_intTourCommentID;
	...
	/// <summary>
		...
		/// <summary>
		/// Gets all TourComments from the database, using the default connection string, into a SQLDataReader
		/// </summary>
		/// <returns>The SQLDataReader With the TourComments</returns>
		public static SqlDataReader GetAllTourCommentsReader() {
			SqlConnection conn = DBHelper.getConnection();
			SqlCommand cmd = DBHelper.getSprocCmd("proc_TourComments__sel_all", conn);
		...
		public static DataSet GetAllTourCommentsDataSet() {
            SqlConnection conn = DBHelper.getConnection();
            SqlCommand cmd = DBHelper.getSprocCmd("proc_TourComments__sel_all", conn);
		...
		/// <summary>
		/// Gets all TourComments from the database, using the default connection string, into a hashtable SQLReader
		/// </summary>
		/// <returns></returns>
		public static Hashtable GetAllTourComments() {
			SqlDataReader dr = GetAllTourCommentsReader();
			return ConvertReaderToHashTable(dr);
		}
		
		/// <summary>
		/// Creates TourComments for records In the prefilled DataReader, And puts them into a HashTable
		/// </summary>
		/// <param name="dr">The DataReader prefilled With the TourComments records</param>
		/// <returns>The Hashtable containing TourComments objects And their ID As key.</returns>
		private static Hashtable ConvertReaderToHashTable(SqlDataReader dr) {
			Hashtable result = new Hashtable();
			while (dr.Read()) {
	            TourComments myTourComments = new TourComments();
            
            myTourComments.m_intTourCommentID = dr.GetSqlInt32(0);
            myTourComments.m_intTourID = dr.GetSqlInt32(1);
            myTourComments.m_intCustomerID = dr.GetSqlInt32(2);
            myTourComments.m_intLanguageID = dr.GetSqlInt32(3);
            myTourComments.m_strOriginalComment = dr.GetSqlString(4);
            myTourComments.m_strApprovedComment = dr.GetSqlString(5);
            myTourComments.m_intRating = dr.GetSqlInt32(6);
            myTourComments.m_dtCreateDate = dr.GetSqlDateTime(7);
            myTourComments.m_bolisConfirmed = dr.GetSqlBoolean(8);
            myTourComments.m_dtConfirmationDate = dr.GetSqlDateTime(9);
            myTourComments.m_intConfirmedBy = dr.GetSqlInt32(10);
            myTourComments.m_bolisActive = dr.GetSqlBoolean(11);
            
            result.Add(myTourComments.TourCommentID, myTourComments);
		}
		...
		public PK PrimaryKey {
			get {
				PK pk;
				pk.TourCommentID = m_intTourCommentID;
				return pk;
			}
		}
			/// <summary>
		/// The TourCommentID column in the DB
		/// </summary>
		public int TourCommentID {
			get {
				return (int)m_intTourCommentID;
			}
			set {
				m_intTourCommentID = value;
				m_bIsDirty = true;
			}
		}
			/// <summary>
		...
		public bool Load(PK pk) {
			return Load(pk.TourCommentID.Value);		}
	
		/// <summary>
		/// Fills the member variables of the Object from the DB based on the primary key, returns true if success.
		/// </summary>
		/// <param name="intTourCommentID"> TourCommentID key value</param>
	/// <returns>true if success</returns>
		public bool Load(Int32 intTourCommentID) {	
			//	construct new connection and command objects
			SqlConnection conn = DBHelper.getConnection();
			SqlCommand cmd = DBHelper.getSprocCmd("proc_TourComments__sel", conn);
		
			SqlParameter param;
		
				//	Add params
	        // parameter for TourCommentID column
        param = new SqlParameter("@TourCommentID", SqlDbType.Int, 4);
        param.Direction = ParameterDirection.Input;
        param.Value = intTourCommentID;
		...
		//	open connection
				...
				if(reader.Read()) {
					...
					//	set member values
		            m_intTourCommentID = reader.GetSqlInt32(0);
            ...
            m_bolisActive = reader.GetSqlBoolean(11);
					...
					//	set key values
		            m_intTourCommentID = intTourCommentID;
	}
		...
		private int ins() {
			...
			if(this.m_Transaction != null) {
							cmd = DBHelper.getSprocCmd("proc_TourComments__ins", conn, this.m_Transaction);
			} else {
							cmd = DBHelper.getSprocCmd("proc_TourComments__ins", conn);
			}
        ...
        param = new SqlParameter("@TourCommentID", SqlDbType.Int, 4);
        param.Direction = ParameterDirection.Input;
        param.Value = m_intTourCommentID;
        cmd.Parameters.Add(param);
		...
		private int upd() {
			...
			SqlCommand cmd;
			if(this.m_Transaction != null) {
				cmd = DBHelper.getSprocCmd("proc_TourComments__upd", conn, this.m_Transaction);
			} else {
				cmd = DBHelper.getSprocCmd("proc_TourComments__upd", conn);
			}
		...
		private int del() {
			...
			SqlCommand cmd;
			if(this.m_Transaction != null) {
				cmd = DBHelper.getSprocCmd("proc_TourComments__del", conn, this.m_Transaction);
			} else {
				cmd = DBHelper.getSprocCmd("proc_TourComments__del", conn);
			}
		...
		/// <returns>true if success</returns>
		public static bool operator ==(TourComments t1, TourComments t2) {
			//	compare values
			if(t1.m_intTourCommentID != t2.m_intTourCommentID) {
				return false;	//	because "TourCommentID" values are Not equal
			}
		...
		public override string ToString() {
			System.Text.StringBuilder retValue = new System.Text.StringBuilder("Keys:\n");		
				retValue.Append(" TourCommentID = \"");
			retValue.Append(m_intTourCommentID);
			retValue.Append("\"\n");
		...
		public override bool Equals(System.Object o) {
			//	check the type of o
			if (!(o is TourComments)) {
				//	types Not the same, return false
				return false;
			}
	
			//	compare objects And return retValue
			return this == (TourComments)o;
		}
