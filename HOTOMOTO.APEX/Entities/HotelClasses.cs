...
private int m_intLanguageID;
	...
	/// <summary>
		...
		/// Default constructor.
		/// </summary>
		public HotelClasses(int LanguageID) {
			Init();	//	init Object
		...
		/// <summary>
		/// Gets all HotelClasses from the database, using the default connection string, into a SQLDataReader
		/// </summary>
		/// <returns>The SQLDataReader With the HotelClasses</returns>
		public static SqlDataReader GetAllHotelClassesReader(int LanguageID) {
			SqlConnection conn = DBHelper.getConnection();
			SqlCommand cmd = DBHelper.getSprocCmd("proc_HotelClasses__sel_all", conn);
		...
		public static DataSet GetAllHotelClassesDataSet(int LanguageID) {
            SqlConnection conn = DBHelper.getConnection();
            SqlCommand cmd = DBHelper.getSprocCmd("proc_HotelClasses__sel_all", conn);
		...
		/// <summary>
		/// Gets all HotelClasses from the database, using the default connection string, into a hashtable SQLReader
		/// </summary>
		/// <returns></returns>
		public static Hashtable GetAllHotelClasses(int LanguageID) {
			SqlDataReader dr = GetAllHotelClassesReader(LanguageID);
			return ConvertReaderToHashTable(dr, LanguageID);
		}
		
		/// <summary>
		/// Creates HotelClasses for records In the prefilled DataReader, And puts them into a HashTable
		/// </summary>
		/// <param name="dr">The DataReader prefilled With the HotelClasses records</param>
		/// <returns>The Hashtable containing HotelClasses objects And their ID As key.</returns>
		private static Hashtable ConvertReaderToHashTable(SqlDataReader dr, int LanguageID) {
			Hashtable result = new Hashtable();
			while (dr.Read()) {
	            HotelClasses myHotelClasses = new HotelClasses(LanguageID);
            
            myHotelClasses.m_intHotelClassID = dr.GetSqlInt32(0);
            myHotelClasses.m_dtCreateDate = dr.GetSqlDateTime(1);
            myHotelClasses.m_intCreatedBy = dr.GetSqlInt32(2);
            myHotelClasses.m_dtModifyDate = dr.GetSqlDateTime(3);
            myHotelClasses.m_intModifiedBy = dr.GetSqlInt32(4);
            myHotelClasses.m_bolisActive = dr.GetSqlBoolean(5);
            
            result.Add(myHotelClasses.HotelClassID, myHotelClasses);
		}
		...
		public PK PrimaryKey {
			get {
				PK pk;
				pk.HotelClassID = m_intHotelClassID;
				return pk;
			}
		}
			/// <summary>
		...
		public bool Load(PK pk) {
			return Load(pk.HotelClassID.Value);		}
	
		/// <summary>
		/// Fills the member variables of the Object from the DB based on the primary key, returns true if success.
		/// </summary>
		/// <param name="intHotelClassID"> HotelClassID key value</param>
	/// <returns>true if success</returns>
		public bool Load(Int32 intHotelClassID) {	
			//	construct new connection and command objects
			SqlConnection conn = DBHelper.getConnection();
			SqlCommand cmd = DBHelper.getSprocCmd("proc_HotelClasses__sel", conn);
		...
		//	open connection
				...
				if(reader.Read()) {
					...
					//	set member values
		            m_intHotelClassID = reader.GetSqlInt32(0);
            ...
            m_bolisActive = reader.GetSqlBoolean(5);
				...
				cmd = DBHelper.getSprocCmd("proc_HotelClasses__sel_ml_data", conn);
			...
			m_strDescription.Add((int)reader.GetSqlInt32(1), (SqlString)reader.GetSqlString(3));				}
					...
					//	set key values
		            m_intHotelClassID = intHotelClassID;
	}
		...
		private int ins() {
			...
			if(this.m_Transaction != null) {
							cmd = DBHelper.getSprocCmd("proc_HotelClasses__ins", conn, this.m_Transaction);
			} else {
							cmd = DBHelper.getSprocCmd("proc_HotelClasses__ins", conn);
			}
        ...
        cmd.Parameters.Add(param);
			...
			conn.Close();
						...
						HotelClasses_ML.Insert(retValue, langID, this.m_strTitle[langID].ToString(), this.m_strDescription[langID].ToString());
					}
		...
		private int upd() {
			...
			SqlCommand cmd;
			if(this.m_Transaction != null) {
				cmd = DBHelper.getSprocCmd("proc_HotelClasses__upd", conn, this.m_Transaction);
			} else {
				cmd = DBHelper.getSprocCmd("proc_HotelClasses__upd", conn);
			}
        ...
        cmd.Parameters.Add(param);
			...
			conn.Close();
								...
								HotelClasses_ML objHotelClasses_ML = new HotelClasses_ML();
					foreach(int langID in this.m_strDescription.Keys) {
						objHotelClasses_ML.HotelClassID = (int)this.m_intHotelClassID;
						objHotelClasses_ML.LanguageID = langID;
						objHotelClasses_ML.Title = this.m_strTitle[langID].ToString();
objHotelClasses_ML.Description = this.m_strDescription[langID].ToString();
 objHotelClasses_ML.Update();
					}
		...
		private int del() {
			...
			SqlCommand cmd;
			if(this.m_Transaction != null) {
				cmd = DBHelper.getSprocCmd("proc_HotelClasses__del", conn, this.m_Transaction);
			} else {
				cmd = DBHelper.getSprocCmd("proc_HotelClasses__del", conn);
			}
		...
		/// <returns>true if success</returns>
		public static bool operator ==(HotelClasses t1, HotelClasses t2) {
			//	compare values
		...
		public override bool Equals(System.Object o) {
			//	check the type of o
			if (!(o is HotelClasses)) {
				//	types Not the same, return false
				return false;
			}
	
			//	compare objects And return retValue
			return this == (HotelClasses)o;
		}
