...
namespace HOTOMOTO.APEX {
	...
	/// <summary>
		...
		/// <summary>
		/// Gets all Packages from the database, using the default connection string, into a SQLDataReader
		/// </summary>
		/// <returns>The SQLDataReader With the Packages</returns>
		public static SqlDataReader GetAllPackagesReader() {
			SqlConnection conn = DBHelper.getConnection();
			SqlCommand cmd = DBHelper.getSprocCmd("proc_Packages__sel_all", conn);
		...
		public static DataSet GetAllPackagesDataSet() {
            SqlConnection conn = DBHelper.getConnection();
            SqlCommand cmd = DBHelper.getSprocCmd("proc_Packages__sel_all", conn);
		...
		/// <summary>
		/// Gets all Packages from the database, using the default connection string, into a hashtable SQLReader
		/// </summary>
		/// <returns></returns>
		public static Hashtable GetAllPackages() {
			SqlDataReader dr = GetAllPackagesReader();
			return ConvertReaderToHashTable(dr);
		}
		
		/// <summary>
		/// Creates Packages for records In the prefilled DataReader, And puts them into a HashTable
		/// </summary>
		/// <param name="dr">The DataReader prefilled With the Packages records</param>
		/// <returns>The Hashtable containing Packages objects And their ID As key.</returns>
		private static Hashtable ConvertReaderToHashTable(SqlDataReader dr) {
			Hashtable result = new Hashtable();
			while (dr.Read()) {
	            Packages myPackages = new Packages();
            
            myPackages.m_intPackageID = dr.GetSqlInt32(0);
            myPackages.m_dtCreateDate = dr.GetSqlDateTime(1);
            myPackages.m_intCreatedBy = dr.GetSqlInt32(2);
            myPackages.m_dtModifyDate = dr.GetSqlDateTime(3);
            myPackages.m_intModifiedBy = dr.GetSqlInt32(4);
            myPackages.m_bolisActive = dr.GetSqlBoolean(5);
            
            result.Add(myPackages.PackageID, myPackages);
		}
		...
		public bool Load(PK pk) {
			return Load(pk.PackageID.Value);		}
	
		/// <summary>
		/// Fills the member variables of the Object from the DB based on the primary key, returns true if success.
		/// </summary>
		/// <param name="intPackageID"> PackageID key value</param>
	/// <returns>true if success</returns>
		public bool Load(Int32 intPackageID) {	
			//	construct new connection and command objects
			SqlConnection conn = DBHelper.getConnection();
			SqlCommand cmd = DBHelper.getSprocCmd("proc_Packages__sel", conn);
		...
		//	open connection
				...
				if(reader.Read()) {
					...
					//	set member values
		            m_intPackageID = reader.GetSqlInt32(0);
		...
		private int ins() {
			...
			if(this.m_Transaction != null) {
							cmd = DBHelper.getSprocCmd("proc_Packages__ins", conn, this.m_Transaction);
			} else {
							cmd = DBHelper.getSprocCmd("proc_Packages__ins", conn);
			}
		...
		private int upd() {
			...
			SqlCommand cmd;
			if(this.m_Transaction != null) {
				cmd = DBHelper.getSprocCmd("proc_Packages__upd", conn, this.m_Transaction);
			} else {
				cmd = DBHelper.getSprocCmd("proc_Packages__upd", conn);
			}
		...
		private int del() {
			...
			SqlCommand cmd;
			if(this.m_Transaction != null) {
				cmd = DBHelper.getSprocCmd("proc_Packages__del", conn, this.m_Transaction);
			} else {
				cmd = DBHelper.getSprocCmd("proc_Packages__del", conn);
			}
		...
		/// <returns>true if success</returns>
		public static bool operator ==(Packages t1, Packages t2) {
			//	compare values
